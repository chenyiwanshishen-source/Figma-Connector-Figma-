...
> 默认端口 `8788`。若被占用，服务会自动改用 `8789`、`8790` … 等后续端口（最多延展 50 个），启动日志会显示实际地址（`MCP endpoint: http://127.0.0.1:<实际端口>/mcp`）。**如果端口变了，请把上面配置里的端口改成实际端口**——Figma 插件面板会自动探测这些端口，无需手动改；但 MCP 客户端不会自动探测，必须手动同步。
